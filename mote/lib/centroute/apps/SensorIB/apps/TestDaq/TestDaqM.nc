...
module TestDaqM {
  ...
  uses interface StdControl as DaqControl;
  uses interface ADConvert2;
  uses interface SetParam2;
}
implementation {
  ...
  void start_test() {
    ...
    // Set param of port 1
    call SetParam2.setParam(1, 0x08);
    // ask for a conversion on port 1
    call ADConvert2.getData(1);
  ...
  // Return of data
  event result_t ADConvert2.dataReady(int32_t data, uint8_t port) {
    if (data == 0)
      call Leds.redOn();
