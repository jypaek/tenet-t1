...
#define INTERPROCESS_MAX_READ_SIZE 300
  ...
  static client_socket_type_t type_array[MAX_SOCKET_CONNECTIONS];
  ...
  static client_socket_type_t type_array_connection[MAX_SOCKET_CONNECTIONS];
  static int connection_socket_count;

  int GetIdFromSocket(int in_socket, client_type_t *client_type, client_socket_type_t *my_socket_type)
  {
    int i;
  ...
  void AddNewServerConnection(int connection, int id)
  {
    ...
    // currently all sockets for nesc code are unix type
    type_array_connection[connection_socket_count] = CLIENT_TYPE_UNIX;
    connection_socket_count++;
    
  }

  int add_new_client(int socket_id, int id, client_socket_type_t my_socket_type)
  {
    dbg(DBG_USR3, "Adding new client socket: %d socket, %d id, count %d\n",
    ...
    // currently all sockets for nesc code are unix type
    type_array[socket_count] = my_socket_type;
    socket_count++;
    ...
    for (i=0; i<connection_socket_count; i++)
      {
        if (connection_socket_lookup[i] == server_connection)
          {
            // found a match!
            dbg(DBG_USR3, "Found match on server socket %d!\n", server_connection);
...
command result_t StdControl.init() 
{
  ...
  if (emt == NULL)
  ...
  else
    {
      ...
      if (emt->socket_id != 0)
        {
          dbg(DBG_USR3, "Adding client socket %d here!\n", emt->socket_id);
#ifdef USE_TENET
          add_new_client(emt->socket_id, 0, CLIENT_TYPE_TCP); 
#else
          add_new_client(emt->socket_id, 0, CLIENT_TYPE_UNIX);
#endif
        }
    }
  /*
...
event result_t SocketTimer.fired()
{
  ...
  int length_read;
  int out_id;
  client_socket_type_t my_socket_type;
  int ready_sock;
          ...
          if (ret == SELECT_NEW_CLIENT)
          ...
          else if (ret == SELECT_NEW_DATA)
            {
              ...
              out_id = GetIdFromSocket(ready_sock, &client_type, 
                                       &my_socket_type);
              dbg(DBG_USR2, "Got data on a client connection, from socket %d, to id %d, %d bytes, socket type %d, client socket type %d!\n", 
                  ready_sock, out_id, length_read, client_type, 
                  my_socket_type);
              if (client_type == CLIENT_TYPE)
                {
                  dbg(DBG_USR3, "Client type packet!\n");
                  pkt_rcvd((link_pkt_t *)buf_read, length_read, 
                           my_socket_type);
                }
              else if (client_type == SERVER_TYPE)
                {
                  dbg(DBG_USR3, "Server type packet!\n");
                                                      ...
                                                      client_type_t client_type)
{
  ...
  dbg(DBG_USR3, "Sending a packet to a socket %p, length %d, socket %d, id %d!\n",
...
command result_t EmSocketI.ReceiveMsg[uint8_t id](void *msg, int16_t length)
{
  ...
  dbg(DBG_USR3, "Sending a packet to the emstar link %p, length %d!\n",
      msg, length);
...
{
  dbg(DBG_ERROR, "Oh noes, bad id %d\n", id);
  return FAIL;
}
