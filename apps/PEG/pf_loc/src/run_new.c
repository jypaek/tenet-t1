...
inline void do_action_model3(float odom_ang, float odom_tr, float xo, float yo, float to, float *xr, float *yr, float *tr)
	{
	float odom_ang_b, odom_tr_b;
//	float cr1=0.5, cr2=0.3, ct1=0.25, ct2=0.6, sd_t=1.0, sd_r=0.6;
	//float n1, n2;

	float gain_t=0.5, gain_r=0.2;
	float norm_t, norm_r;
	...
	odom_tr_b = odom_tr   + (odom_tr  * norm_t * gain_t);	
	odom_ang_b = odom_ang + (odom_ang * norm_r * gain_r) + (odom_tr  * norm_r * gain_r);	

//	trn = tr     + (tr * norm_t * gain_t);
//	rotn = rot + (rot * norm_r * gain_r) + (tr * norm_t *gain_r);
...
//	part[i][index+1].a = part[i][index].a + rotn;

//	part[i][index+1].x = part[i][index].x + trn * cos(part[i][index+1].a);
//	part[i][index+1].y = part[i][index].y + trn *  sin(part[i][index+1].a);

//	while (part[i][index].a> 3.1415927) part[i][index].a-= 6.2831853;
//	while (part[i][index].a<-3.1415927) part[i][index].a+=6.2831853;
...
//			printf("\n\nOdom x:%.5f, y:%.5f, a:%.5f", position->px, position->py, position->pa);
//			printf("\ndif_x:%.5f, dif_y:%.5f, atan2:%.5f", dif_x, dif_y, atan2(dif_y,dif_x));
//			printf("\nang_o:%.5f, ang_n:%.5f ang_dif:%.5f", ao, position->pa, dif_a);
//			printf("\nrot1:%.5f, rot2:%.5f trn:%.5f\n", rot1, rot2, trn);
...
//				do_action_model2(rot1, rot2, trn, particle[i].x, particle[i].y, particle[i].a, &particle[i].x, &particle[i].y, &particle[i].a);
				...
				do_action_model3(odom_ang, odom_tr, particle[i].x, particle[i].y, particle[i].a, &particle[i].x, &particle[i].y, &particle[i].a);
...
//	init_particles();
	init_particles2(11.70, 3.10, 0);

	client = playerc_client_create(NULL, "localhost", 6665);
	if (playerc_client_connect(client) != 0) printf("\nError: Init Player");
	if (playerc_position_subscribe(position, PLAYER_READ_MODE))
	  	printf("\nError: Init Player");
