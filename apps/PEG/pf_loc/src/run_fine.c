...
//	odom_ang_b = odom_ang + (odom_ang * norm_r * gain_r);	
...
/*	if ((fabs(rot1) > 0.3) && (fabs(rot2) > 0.3))
		{
		rot1b	 = rot1;
		rot2b	 = rot2;
		trnb   = trn;
...
//		playerc_client_read(client);
		...
		if ((int)(dif_x*1000) || (int)(dif_y*1000) || (int)(dif_a*1000))
			...
			printf("\ndif_x:%.5f, dif_y:%.5f, dif_a:%.5f, at2:%.5f\n", dif_x, dif_y, dif_a, at2);
//			printf("\nrot1:%.5f, rot2:%.5f\n\n", rot1, rot2);
//			printf("\nodom_tr:%.5f, odom_ang:%.5f\n\n", odom_tr, odom_ang);
			fflush(stdout);
...
/*
			//old action model
			at2=atan2(dif_y,dif_x);
			dif_a=round_angle(dif_a);

//			if ((dif_y<0.0001) || (dif_x<0.0001))
//				at2=position->pa;
