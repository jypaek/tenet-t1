...
#define BUF_SIZE 65536  /* the size of the image buffer */
...
typedef struct result_item {
    struct result_item *next;   /* linked list pointer */
    int addr;                   /* address of the mote */
    char buffer[BUF_SIZE];      /* buffer pointer to the image data */
    int data_type;              /* image, compressed_image, param, result, etc */
    int num_pkts;               /* total number of packets received from this mote */
    int image_cnt;              /* total number of images collected from this mote */
    int incomplete_image;       /* whether there is an image where transfer is incomplete */
    int recv_bytes;             /* # bytes received for the image currently being transfered */
