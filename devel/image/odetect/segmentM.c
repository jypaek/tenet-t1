...
//{
//_seg_join(G_L)
//if (T) _seg_merge(G_L,G_T)
//elseif (TR) _seg_merge(G_TR,G_L)
//}
//elseif (T)
//{
//    _seg_join(G_T)
...
//{
//_seg_join(G_TL)
//if (TR) _seg_merge(G_TL,G_TR)
//}
//elseif (TR) _seg_join(G_TR)
...
/******************************************/
    ...
    uint16_t s_get_memloc(uint8_t row, uint8_t col) {
        return  (objectMatrix->cols)*row+col;
    }

    llnode *_seg_getllnode() {
        llnode *head;
    ...
    int _seg_create(uint8_t myrow, uint8_t mycol) {
        llnode *myllnode;
        myllnode = _seg_getllnode();
        if (myllnode == NULL) return 0;
        myllnode->row = myrow;
        myllnode->col = mycol;
        myllnode->next = NULL;
        ...
        if (currentGroupNumber == MAXIMUM_NUMBER_LISTS) return 0;
        //later we test this following logic to release memory

        objectMatrix->data.ptr8[s_get_memloc(myrow,mycol)] = currentGroupNumber; //writing group number into the associated pixel
    ...
    int _seg_join(uint8_t joinedGroup, uint8_t myrow, uint8_t mycol)
    {
        llnode *myllnode;
        myllnode = _seg_getllnode();
        if (myllnode == NULL) return 0;
        ...
        objectMatrix->data.ptr8[s_get_memloc(myrow,mycol)] = joinedGroup;
    ...
    void _seg_merge(uint8_t group1,uint8_t group2)
    {   
        llnode *myllnode;
        llnode *lastllnode;
        ...
        {     
            ...
            while((myllnode->next)!= NULL) {   
                objectMatrix->data.ptr8[s_get_memloc(myllnode->row,myllnode->col)] = group1;
        ...
        {                 
            ...
            while((myllnode->next)!= NULL)            
            {   
                objectMatrix->data.ptr8[s_get_memloc(myllnode->row,myllnode->col)] = group2;
                myllnode = myllnode->next;
            };
            //althought the next element is null but this is actualy the last valid link in the link list
            //so this one should also be regrouped. of course no more since next one is null.
    ...
    void _seg_clearMatrix() {
        ...
        for (clearIndex = 1;clearIndex<MAXIMUM_NUMBER_LISTS;clearIndex++)
        {
            ...
            if ((linkedListSet[clearIndex].linkedList != NULL) && (linkedListSet[clearIndex].memberNumber <= blobFiterThreshold))
            {
                ...
                while((myllnode->next)!= NULL)
                {
                    objectMatrix->data.ptr8[s_get_memloc(myllnode->row,myllnode->col)] = 0;
                    myllnode = myllnode->next;
                }
                //we set every pixel in the object matrix to zero for the groups we remove
                objectMatrix->data.ptr8[s_get_memloc(myllnode->row,myllnode->col)] = 0;
    ...
    int segment_segment(CYCLOPS_Matrix* myObjectMatrix, linkedListSets* myllSet, llnode* mI, uint16_t myBlobFiterThreshold, uint8_t myThreshold)
    {
        ...
        if (objectMatrix->depth != CYCLOPS_1BYTE) return 0;

        //The first element
        if (objectMatrix->data.ptr8[0] > myThreshold) {
            //Do not need to clear matrix since no elements were created
            if (_seg_create(0,0) == 0) return 0;
        }
        else
        ...
        for (c = 1; c < (objectMatrix->cols); c++) {   
            if (objectMatrix->data.ptr8[c]>myThreshold) {
                if (objectMatrix->data.ptr8[c-1]) {
                    if (_seg_join(objectMatrix->data.ptr8[c-1],0,c) == 0) {
                        _seg_clearMatrix();
                        return 0;
                    }
                } else { 
                    if (_seg_create(0,c) == 0) {
                        _seg_clearMatrix();
                        return 0;
                    }
                }
        ...
        for (r = 1; r < (objectMatrix->rows); r++) {            
            ...
            if (objectMatrix->data.ptr8[s_get_memloc(r,c)]>myThreshold) {
                //checking the top pixel
                if (objectMatrix->data.ptr8[s_get_memloc(r-1,c)]) {
                    if (_seg_join(objectMatrix->data.ptr8[s_get_memloc(r-1,c)],r,c) == 0) {
                        _seg_clearMatrix();
                        return 0;
                    }
                }
                //now checking the top right pixel
                else if (objectMatrix->data.ptr8[s_get_memloc(r-1,c+1)]) {
                    if (_seg_join(objectMatrix->data.ptr8[s_get_memloc(r-1,c+1)],r,c) == 0) {
                        _seg_clearMatrix();
                        return 0;
                    }
                }
                ...
                objectMatrix->data.ptr8[s_get_memloc(r,c)] = 0;
            ...
            for (c = 1; c < (objectMatrix->cols-1); c++) {
                ...
                if (objectMatrix->data.ptr8[s_get_memloc(r,c)]>myThreshold) {
                    ...
                    if (objectMatrix->data.ptr8[s_get_memloc(r,c-1)]) {
                        if (_seg_join(objectMatrix->data.ptr8[s_get_memloc(r,c-1)],r,c) == 0) {
                            _seg_clearMatrix();
                            return 0;
                        }
                        //checking the top pixel to merge top and left 
                        if (objectMatrix->data.ptr8[s_get_memloc(r-1,c)]) {
                            _seg_merge(objectMatrix->data.ptr8[s_get_memloc(r-1,c)],objectMatrix->data.ptr8[s_get_memloc(r,c-1)]);
                        }
                        //checking the top right pixel to merge top right and left
                        else if (objectMatrix->data.ptr8[s_get_memloc(r-1,c+1)]) {
                            _seg_merge(objectMatrix->data.ptr8[s_get_memloc(r-1,c+1)],objectMatrix->data.ptr8[s_get_memloc(r,c-1)]);
                        }
                    }
                    //now checking the top pixel
                    else if (objectMatrix->data.ptr8[s_get_memloc(r-1,c)]) {
                        if (_seg_join(objectMatrix->data.ptr8[s_get_memloc(r-1,c)],r,c) == 0) {
                            _seg_clearMatrix();
                            return 0;
                        }
                    ...
                    //now checking the top left
                        ...
                        if (objectMatrix->data.ptr8[s_get_memloc(r-1,c+1)]) {
                            _seg_merge(objectMatrix->data.ptr8[s_get_memloc(r-1,c+1)],objectMatrix->data.ptr8[s_get_memloc(r-1,c-1)]);
                        }
                    }
                    //now checking the top right pixel
                    else if (objectMatrix->data.ptr8[s_get_memloc(r-1,c+1)]) {
                ...
                else objectMatrix->data.ptr8[s_get_memloc(r,c)] = 0;
            }
